        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-12">
            <div class="card" >
                <div id="Listados" class="card-header d-flex justify-content-between">
                    <h5><a class="text-dark2">Listado de Idiomas</a></h5>
                    <a href="{{route('languages.aCreate')}}" class="btn btn-outline-secondary btn-sm text-white">+ Crear Idioma
                </a>
                </div>
                        ...
                        <form method="POST" action="">
                            ...
                            <div class="input-group mb-3">
                                <div class="input-group-prepend" style="width: 80%">
                                    <input type="text" name="dataFiltro" id="dataFiltro" class="form-control" value="@isset($dataFiltro) {{$dataFiltro}} @endisset" placeholder="Buscar por nombre o código del idioma">
                                </div>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @if (count($languages)>0)
                                <?php $contador = 0; ?>
                                @foreach ($languages as $dato)
                                    <tr>
                                        <td class="indice">{{++$contador}}</td>
                                        <td>{{$dato->isoCode}}</td>
                                        <td>{{$dato->name}}</td>
                                        <td class="text-center"> 
                                            <a href="{{ route('languages.aShowForm',[$dato->idLanguage])}}" class="btn btn-danger"><i class="fa fa-edit">Editar</i></a>
                                            <form method="POST" action="{{route('languages.aDelete',$dato)}}" style="display: inline-block;">
                                                {{method_field('delete')}}
                                                {!! csrf_field() !!}
                                                <input type="hidden" name="idLanguage" id="idLanguage" value="{{$dato->idLanguage}}">
                            ...
                            @else
                                <td class="font-italic text-center" colspan="3">
                                    No existen idiomas registrados
                                </td>
                            @endif
                        </tbody>
                    </table>
